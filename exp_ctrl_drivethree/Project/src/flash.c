...
void flash_init(void)
{
    ...
    if (motolocal.moudlenum != 0xFFFF) {
        ...
        Set_Mot_Roll_Paras(MOT1, 4, DIR_CW, 0);
        Set_Mot_Roll_Paras(MOT2, 4, DIR_CW, 0);
...
#endif
    Set_Mot_Roll_Paras(MOT3, 6, DIR_CW, 2);
...
void flash_write_buff(u16* buff, u16 len)
{
    ...
    {
        for (i = 0; i < len; i++) {
            FLASH_ProgramHalfWord(UserParaAddress + 2 * i, buff[i]);
        }
    }
    FLASH_Lock();
}
